...
//상속을 활용하면 물려받은 것들은 그래도 쓰면되고, 거기에 덧붙여 새로운 것만 만들면 되므로 그만큼 노력과 시간이 세이브됨.
//이떄 물려받게되는 원본 클래스를 부모 클래스(Parent class) 또는 슈퍼 클래스(Super class)라고 부른다.
...
//[2] 상속의 장점
// 가장 큰 장점 > 재활용성                 ex) 부모님이 타시던 오토바이를 물려받았는데  자식은 오토바이를 배달용으로 사용한다.
// 완전히 새로운 것을 만드는 것이 아니라 기존 부모로 부터 상속을 받아 필요한 것만 추가로 더해서 만드는 것.
//부모클래스에 정의되어져 있는 멤버필드(변수) 나 메서드 들을 그대로 상속 담아 사용하면 된다.
//상속받은 메서드라 해도 필요에 따라서 자식클래스에서 용도를 변경해서 사용하는 것도 가능.


//[3] 상속의 사용
//기존 부모클래스를 확장한다는 개념. > extends 키워드를 사용.
//부모 클래스의 멤버 필드,메서드는 상속이 가능하나 생성자는 상속이 안된다.
//부모 클래스의 접근 제한자 private 인 경우에는 아무리 자식 클래스가 상속을 받았다 하더라도 접근 불가능.
